...
def select_n_clusters(data, data_pca, n_clusters_range):
    ...
    for n in n_clusters_range:
        kmeans = cluster.KMeans(n_clusters=n).fit(data_pca)
        score = get_score(data, kmeans)
        scores.append(score)
    for i, score_function in enumerate(['SSE', 'silhouette_score', 'calinski_harabaz_score']):
        plt.subplot(1, 3, i+1)
